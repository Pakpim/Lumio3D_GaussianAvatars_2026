...
from gaussian_renderer import render, render_fastgs, FASTGS_AVAILABLE
...
def render_set(dataset : ModelParams, name, iteration, views, gaussians, pipeline, background, render_mesh, use_fastgs=False, mult=0.5):
    ...
    for idx, view in enumerate(tqdm(views_loader, desc="Rendering progress")):
        ...
        if use_fastgs and FASTGS_AVAILABLE:
            rendering = render_fastgs(view, gaussians, pipeline, background, mult=mult)["render"]
        else:
...
if __name__ == "__main__":
    ...
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_val, args.skip_test, args.render_mesh, use_fastgs=opt_args.use_fastgs, mult=opt_args.mult)