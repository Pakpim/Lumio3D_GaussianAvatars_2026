...
def render_fastgs(viewpoint_camera, pc: Union[GaussianModel, FlameGaussianModel], pipe, bg_color: torch.Tensor,
    ...
    # FastGS uses (N, 4) screenspace points — extra columns store absolute gradients
    screenspace_points = torch.zeros((pc.get_xyz.shape[0], 4), dtype=torch.float32,
    ...
    # --- lumio_base features: backface culling + depth map ---
    opacity_ = opacity.clone()
